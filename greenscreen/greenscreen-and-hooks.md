# Green-screen backgrounds + hook text — Week of Aug 3–9, 2026
**Anchor: "The Fed held. Your mortgage went up anyway."**

> **⚠ PIPELINE NOTE FOR THIS RUN:** Claude in Chrome was **not connected**, so **Pexels b-roll could not be downloaded** and **Atlas Berry could not be reviewed** this week. Every background below is a **generated TGW data tile** (Pillow, 1080×1920, locked caption-box spec) plus a Ken Burns clip built from the cold-open tile. The tiles carry OUR numbers — which is their standing job per CLAUDE.md step 7 — but the mood/subject footage layer is missing. **If Wolf wants real footage under these beats, reconnect the Chrome extension and re-run step 7, or drop in Pexels clips manually behind the same captions.**

**The locked box spec (unchanged):** dark navy `#131A22` panel · `#243140` border · gold `#F5C451` Title-Case text · key number in mint `#34E5A3` · upper-third centered · Poppins Bold · "educational content only — not financial advice" on every asset.

**Caption scheme (Jul 10, 2026):** the INTRO tile carries the full TGW hook box; each Pt tile carries a mint `PT N` kicker + gold Title-Case step title, so the headers read as an on-screen progress bar matching the Sora story map. Pt 3 is always the payoff step.

---

## 🔴 TUESDAY, AUG 4 — P6 · NEWS / REACT
**Cold-open text (≤5 words):** `$358 A Month Apart`
**Sustained caption (≤12 words):** `There's $358 A Month Between You And Your First House`

| # | File | Lands on | Beat |
|---|---|---|---|
| 1 | `TUE/TUE_01_cold-open.png` | 0:00–0:02 — "Three hundred fifty-eight dollars a month." | COLD OPEN |
| 2 | `TUE/TUE_kenburns.mp4` | 0:02–0:10 — the stakes / Fed held, mortgage rose | STAKES |
| 3 | `TUE/TUE_02_pt1-fed-vs-bond.png` | 0:10–0:28 — "The Fed doesn't set your mortgage rate" | RISING · Pt 1 header |
| 4 | `TUE/TUE_03_pt2-true-cost.png` | 0:28–0:48 — $459,710 total interest | RISING · Pt 2 header |
| 5 | `TUE/TUE_04_pt3-payoff.png` | 0:52–1:20 — $358/mo · $128,926 | **PAYOFF · Pt 3 header** |
| 6 | *(live)* `screenshare/rate-ladder-dashboard.html` | 0:52+ — run both tiers on camera | payoff proof |

**Cut order:** cold-open tile → Ken Burns under the stakes → Pt 1 header on the but/therefore turn → Pt 2 header → *zoom on the re-hook* → Pt 3 header → back to the cold-open tile for the loop-close.
**Loop-close visual:** return to `TUE_01_cold-open.png` on the literal callback *"three hundred fifty-eight dollars a month."*

---

## 📈 THURSDAY, AUG 6 — P1 · TRADE (chart evergreen)
**Cold-open text (≤5 words):** `Two Lines. That's It.`
**Sustained caption (≤12 words):** `2 Lines Decide If You Buy Scared Or On A Plan`

| # | File | Lands on | Beat |
|---|---|---|---|
| 1 | `THU/THU_01_cold-open.png` | 0:00–0:02 — "Two lines. That's the whole thing." | COLD OPEN |
| 2 | `THU/THU_kenburns.mp4` | 0:02–0:10 — why people panic-sell | STAKES |
| 3 | `THU/THU_02_pt1-what-they-are.png` | 0:10–0:26 — memory, not magic; touches make a level | RISING · Pt 1 |
| 4 | `THU/THU_03_pt2-draw-them.png` | 0:26–0:46 — 1Y range, horizontal, closing prices | RISING · Pt 2 |
| 5 | `THU/THU_04_pt3-payoff.png` | 0:50–1:18 — 744.99 / 700.39 / close 747.03 | **PAYOFF · Pt 3** |
| 6 | *(live)* `screenshare/chart-levels-dashboard.html` | 0:50+ — draw the zones on the real chart | payoff proof |

**Cut order:** cold-open → Ken Burns → Pt 1 → Pt 2 → *zoom on re-hook* → Pt 3 + live chart → cold-open tile on "**two lines**."
**Note:** the dashboard's SVG is built from real SPY weekly bars (Apr 27 – Jul 31, 2026) — draw the 5 resistance touches one at a time on camera. That's the moment that teaches it.

---

## ⚡ FRIDAY, AUG 7 — P6 · NEWS / REACT (jobs day — the allowed 4th drop)
**Cold-open text (≤5 words):** `8:30 This Morning`
**Sustained caption (≤12 words):** `One Number Just Moved Your Rate, Your Raise And Your Rent`

| # | File | Lands on | Beat |
|---|---|---|---|
| 1 | `FRI/FRI_01_cold-open.png` | 0:00–0:02 — "[FILL LIVE] jobs. 8:30 this morning." | COLD OPEN |
| 2 | `FRI/FRI_kenburns.mp4` | 0:02–0:10 — moves your mortgage before your paycheck | STAKES |
| 3 | `FRI/FRI_02_pt1-two-surveys.png` | 0:10–0:26 — establishment vs household | RISING · Pt 1 |
| 4 | `FRI/FRI_03_pt2-run-rate.png` | 0:26–0:46 — +36,000/month · the revision paragraph | RISING · Pt 2 |
| 5 | `FRI/FRI_04_pt3-payoff.png` | 0:50–1:15 — wages 3.5% − core 3.3% = 0.2% | **PAYOFF · Pt 3** |
| 6 | ⛔ `FRI/FRI_05_FILL-LIVE.png` | **REPLACE BEFORE POSTING** | FILL-LIVE frame |

**⛔ FILL-LIVE PROTOCOL — this is the one asset that must not ship as-is.**
`FRI_05_FILL-LIVE.png` carries blanks and a red **VERIFY BEFORE POSTING** banner **by design**. Friday 8:30–9:15am ET: pull payrolls, unemployment rate, average hourly earnings y/y, and the May/June revisions from `bls.gov/news.release/empsit.nr0.htm`, rebuild the tile with the real figures, remove the banner, and **ship by 10:00am ET.** Three separate weeks in Performance Intelligence show a late reveal loses 60–75% of its reach.

---

## 🤝 SATURDAY, AUG 8 — P5 · COLLAB / GUEST-REACTION (IG-FIRST)
**Cold-open text (≤5 words):** `"Wait For Rates To Drop"`
**Sustained caption (≤12 words):** `I Ran The Numbers On "Wait For Rates" — It's Not Close`

| # | File | Lands on | Beat |
|---|---|---|---|
| 1 | `SAT/SAT_01_cold-open.png` | 0:00–0:03 — the creator's claim, on screen | COLD OPEN |
| 2 | `SAT/SAT_kenburns.mp4` | 0:03–0:12 — this is your first place | STAKES |
| 3 | `SAT/SAT_02_pt1-cost-of-waiting.png` | 0:12–0:30 — $14/mo for a whole year of waiting | RISING · Pt 1 |
| 4 | `SAT/SAT_03_pt2-refinance.png` | 0:30–0:50 — rate is rentable, price is permanent | RISING · Pt 2 |
| 5 | `SAT/SAT_04_pt3-verdict.png` | 0:54–1:22 — 25×+ · $14 vs $358 | **PAYOFF · Pt 3** |

**📌 SAT_01 has a deliberate placeholder.** The sub-line reads `[ CREATOR CLAIM GOES HERE, VERBATIM ]`. On record day, screenshot the chosen clip and overlay the creator's **actual words**, credited — that's the EYL pattern Sora logged (§6): the caption box carries the *contestable claim*, never the topic.
**⚠ Never attribute a claim to a real creator that they did not make.** If no suitable clip is found, use the unattributed framing *"The take going around right now:"* instead.
**This week's panel creator:** **Aristotle Investments** (first-person "how I actually did it" receipts), rotating in after EYL (Jul 20/24) and Caleb Hammer (Jul 27–Aug 2).

---

## 🎙 SUNDAY, AUG 9 — PODCAST (the anchor)
**Cold-open text (≤5 words):** `The Fed Held Anyway`
**Sustained caption (≤12 words):** `The Fed Held. Your Mortgage Went Up $128,926 Anyway.`

| # | File | Lands on |
|---|---|---|
| 1 | `PODCAST/PODCAST_01_cold-open.png` | 0:00–1:30 cold open |
| 2 | `PODCAST/PODCAST_kenburns.mp4` | episode intro bed / Sunday teaser |
| 3 | `PODCAST/PODCAST_02_seg1-the-vote.png` | Segment 1 — the 9–3 vote |
| 4 | `PODCAST/PODCAST_03_seg2-the-ladder.png` | Segment 2 — the rate ladder |
| 5 | `PODCAST/PODCAST_04_seg3-the-gap.png` | Segment 3 — the $358 gap |
| 6 | `PODCAST/PODCAST_05_seg4-payoff.png` | Segment 4 — payoff tile, $14 vs $358 |

**Screen-share cues:** `jobs-week-dashboard.html` panels 1–4 map one-to-one to segments 1–4; `rate-ladder-dashboard.html` runs live in Segment 3; `chart-levels-dashboard.html` is the optional chart insert.

---

## Hook-doctrine check (every hook above)

| Day | Viewer is the subject? | Specific number? | Life outcome? | Earned by the math? |
|---|---|---|---|---|
| TUE | ✅ your first house | ✅ $358/mo · $128,926 | ✅ first home | ✅ tier calculator on screen |
| THU | ✅ how *you* buy | ✅ 2 lines · 744.99 / 700.39 | ✅ buying on a plan, not scared | ✅ live chart, real touches |
| FRI | ✅ your rate/raise/rent | ✅ 8:30 · +36,000 · 0.2% | ✅ your actual raise | ✅ pay-stub subtraction |
| SAT | ✅ your first place | ✅ $14 vs $358 · 25×+ | ✅ first home | ✅ both sides priced |
| SUN | ✅ your family's house | ✅ $128,926 | ✅ career-stage / family (35–54 cut) | ✅ full segment of math |

**Banned and avoided:** no "quit your 9-5", no price predictions, no "before it's too late", no emotional hook without a number, no borrow-to-invest framing anywhere.

*Educational content only — not financial advice. All figures computed and verified during the Aug 3, 2026 run.*
